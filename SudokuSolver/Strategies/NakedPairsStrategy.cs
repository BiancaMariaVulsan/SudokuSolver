    ...
    public class NakedPairsStrategy : ISudokuStrategy
    {
        ...
        public int[,] Solve(int[,] sudokuBoard)
        {
            for (int row = 0; row < sudokuBoard.GetLength(0); row++)
            {
                ...
                {
                    EliminateNakedPairFromOtherInRow(sudokuBoard, row, col);
                    EliminateNacedPairFromOtherInCol(sudokuBoard, row, col);
                    EliminateNakedPairFromOtherInBlock(sudokuBoard, row, col);
                }
            }
            return sudokuBoard;
        }

        private void EliminateNakedPairFromOtherInBlock(int[,] sudokuBoard, int givenRow, int givenCol)
        {
            if (!HasNackedPairInBlock(sudokuBoard, givenRow, givenCol)) return;
        ...
        private void EliminateNacedPairFromOtherInCol(int[,] sudokuBoard, int givenRow, int givenCol)
        {
            if (!HasNackedPairInCol(sudokuBoard, givenRow, givenCol)) return;
        ...
        private void EliminateNakedPairFromOtherInRow(int[,] sudokuBoard, int givenRow, int givenCol)
        {
            if (!HasNackedPairInRow(sudokuBoard, givenRow, givenCol)) return;
        ...
        private bool HasNackedPairInRow(int[,] sudokuBoard, int givenRow, int givenCol)
        {
            for (int col = 0; col < sudokuBoard.GetLength(0); col++)
            {
        ...
        private bool HasNackedPairInCol(int[,] sudokuBoard, int givenRow, int givenCol)
        {
            for (int row = 0; row < sudokuBoard.GetLength(0); row++)
            {
        ...
        private bool HasNackedPairInBlock(int[,] sudokuBoard, int givenRow, int givenCol)
        {
            for (int row = 0; row < sudokuBoard.GetLength(0); row++)
            {
                for (int col = 0; col < sudokuBoard.GetLength(1); col++)
